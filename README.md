#  Привет! Меня зовут Наталия!

Я начинающий специалист в ручном тестировании. Мой путь в IT начался с роли администратора IT-проектов в 2018 году, где я получала ценный опыт: коммуникация с IT-командой, анализ отчётов, взаимодействие с клиентами и работа с документацией.

Я поняла, что мне важно не просто сопровождать IT-процессы, а быть внутри: исследовать системы, работать с базами данных и влиять на качество. Сейчас я стремлюсь развивать навыки тестирования API, баз данных и активного использования различных инструментов.


---

##  Tools by Testing Type

###  Development & IDEs
<p align="left">
   <img src="https://cdn.simpleicons.org/xcode/147EFB" height="40" alt="Xcode" />
  <img src="https://cdn.simpleicons.org/androidstudio/3DDC84" height="40" alt="Android Studio" />
  <img src="https://cdn.simpleicons.org/visualstudio/5C2D91" height="40" alt="Visual Studio" />
  <img src="https://cdn.simpleicons.org/visualstudiocode/007ACC" height="40" alt="Visual Studio Code" />
</p>

###  Design & Prototyping
<p align="left">
  <img src="https://cdn.simpleicons.org/figma/F24E1E" height="40" alt="Figma" />
</p>

###  API & Backend Testing
<p align="left">
  <img src="https://cdn.simpleicons.org/postman/FF6C37" height="40" alt="Postman" />
  <img src="https://cdn.simpleicons.org/mysql/4479A1" height="40" alt="MySQL" />
  <img src="https://cdn.simpleicons.org/mongodb/47A248" height="40" alt="MongoDB" />
  <img src="https://cdn.simpleicons.org/sqlite/003B57" height="40" alt="SQL" />
</p>

###  DevOps & Source Control
<p align="left">
  <img src="https://cdn.simpleicons.org/git/F05032" height="40" alt="Git" />
  <img src="https://cdn.simpleicons.org/github/181717" height="40" alt="GitHub" />
  <img src="https://cdn.simpleicons.org/gnubash/4EAA25" height="40" alt="Bash" />
</p>

###  Project & Bug Tracking
<p align="left">
  <img src="https://cdn.simpleicons.org/youtrack/2E9FFF" height="40" alt="YouTrack" />
</p>

###  Web Debugging & Browser Tools
<p align="left">
  <img src="https://cdn.simpleicons.org/googlechrome/4285F4" height="40" alt="Chrome DevTools" />
</p>


## 📂 Примеры работ и активностей

🔹 [Тестовая документация (чек-листы, тест-кейсы, баг-репорты)](https://github.com/твоя-ссылка-на-проект)  
🔹 [Коллекции Postman для API-тестирования](https://github.com/твоя-ссылка-на-проект)  
🔹 [Исследовательское тестирование веб-приложений](https://github.com/твоя-ссылка-на-проект)  

---

## 📫 Контакты

💼 [LinkedIn](https://www.linkedin.com/in/natalia-gavrilenko-80b3b364)  
📧 n.gavrilenko@hotmail.com

---

## 🌱 Немного обо мне

🎯 Развиваюсь в области тестирования
📚 Интересуюсь тестированием API и баз данных  
💡 Люблю чёткие процессы, понятные требования и качественные интерфейсы  
☕ Кофе, внимание к деталям и баг-репорты — мой стиль 😄  

---

⭐️ *Если тебе понравился мой профиль — поддержи его звёздочкой!*

⭐️ *Если тебе понравились мои проекты — поставь звёздочку!*
